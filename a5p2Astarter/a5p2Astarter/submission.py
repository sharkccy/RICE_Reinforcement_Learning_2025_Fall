...
def lr_schedule(progress_remaining):
    return START_LR * progress_remaining + END_LR * (1 - progress_remaining)

def sigma_schedule(progress_remaining):
    return START_SIGMA * progress_remaining + END_SIGMA * (1 - progress_remaining)
...
class EpisodeRewardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if "episode" in self.locals["infos"][0]:
            ...
            if ep_reward >= 0:
                self.model.save(f"ddpg_bipedalwalker_{self.num_timesteps}_steps_reward_{ep_reward:.2f}.zip")
...
class DDPGTrainer:
    ...
    def __init__(self, env: gym.Env):
        ...
        # Initial sigma for action noise
        self.action_noise = NormalActionNoise(mean=np.zeros(self.n_actions), sigma=self.sigma_schedule(1.0) * np.ones(self.n_actions))
        self.batch_size = 256
        self.tau = 0.005            # target network update rate, theta target = tau * theta local + (1 - tau) * theta target
        self.gamma = 0.995
        self.train_freq = 1
        self.verbose = 1
        self.seed = RANDOM_SEED
        
        self.tensorboard_log = f"./tensorboard_logs_lr{START_LR}_start_sigma{START_SIGMA}_bs{self.batch_size}_tau{self.tau}_gamma{self.gamma}/"
    ...
    def train(self, total_timesteps: int) -> List[float]:
        ...
        # Update action noise sigma during training (optional, for advanced usage)
        # SB3 does not natively support dynamic noise, but you can manually update it in a custom callback if needed.
        print(f"Model seed: {self.model.seed}")
        self.model.learn(total_timesteps=total_timesteps, callback=reward_callback)
        ######## PUT YOUR CODE HERE ########
...
if __name__ == "__main__":
    ...
    ######## PUT YOUR CODE HERE ########
    if CONTINUE:
        trainer.model.load("./starting_model")
